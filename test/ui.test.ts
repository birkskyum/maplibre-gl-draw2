...
test('ui buttons with one options.controls', () => {
  ...
  assert.ok(buttons[0].classList.contains('maplibre-gl-draw_line'), 'has line class');
  assert.ok(buttons[0].classList.contains('maplibre-gl-draw_ctrl-draw-btn'), 'has control class');
...
test('ui buttons with all options.controls, no attribution control', async (t) => {
  ...
  assert.ok(buttons[0].classList.contains('maplibre-gl-draw_line'), 'first button has line class');
  assert.ok(buttons[0].classList.contains('maplibre-gl-draw_ctrl-draw-btn'), 'first button has control class');
  assert.equal(buttons[0].parentNode, controlGroup, 'first button is in controlGroup');
  const lineButton = buttons[0];

  assert.ok(buttons[1].classList.contains('maplibre-gl-draw_polygon'), 'second button has polygon class');
  assert.ok(buttons[1].classList.contains('maplibre-gl-draw_ctrl-draw-btn'), 'second button has control class');
  assert.equal(buttons[1].parentNode, controlGroup, 'second button is in controlGroup');
  const polygonButton = buttons[1];
  assert.ok(buttons[2].classList.contains('maplibre-gl-draw_point'), 'third button has point class');
  assert.ok(buttons[2].classList.contains('maplibre-gl-draw_ctrl-draw-btn'), 'third button has control class');
  assert.equal(buttons[2].parentNode, controlGroup, 'third button is in controlGroup');
  const pointButton = buttons[2];
  assert.ok(buttons[3].classList.contains('maplibre-gl-draw_trash'), 'fourth button has trash class');
  assert.ok(buttons[3].classList.contains('maplibre-gl-draw_ctrl-draw-btn'), 'fourth button has control class');
